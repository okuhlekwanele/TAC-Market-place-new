...
import { useTownshipProfiles, TownshipProfile } from '../hooks/useTownshipProfiles';
import { useGoogleSheets } from '../hooks/useGoogleSheets';

export function TownshipProfilesTable() {
  const { profiles, updateProfile, deleteProfile, regenerateAIContent, loading } = useTownshipProfiles();
  const { batchSyncTownshipProfiles, loading: sheetsLoading } = useGoogleSheets();
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('all');
  const [selectedProfile, setSelectedProfile] = useState<TownshipProfile | null>(null);
  ...
  const handleBatchSync = async () => {
    const successCount = await batchSyncTownshipProfiles(profiles);
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Township Profiles</h2>
